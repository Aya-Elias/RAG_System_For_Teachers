...
def inject_theme():
    st.markdown(
        ...
        /* Main pane — same chalkboard as the sidebar */
        [data-testid="stAppViewContainer"] > .main {
            background-color: var(--chalkboard);
            background-image: radial-gradient(rgba(255,255,255,0.03) 1px, transparent 1px);
            background-size: 6px 6px;
        }
        [data-testid="stAppViewContainer"] > .main p, [data-testid="stAppViewContainer"] > .main label,
        [data-testid="stAppViewContainer"] > .main span, [data-testid="stAppViewContainer"] > .main div {
            color: var(--chalk);
        }
        ...
        /* Grade badge — a chalk-drawn circle, the page's signature element */
        .grade-badge {
            display: inline-block; font-family: 'Caveat', cursive; font-size: 1.05rem;
            color: var(--chalk-yellow); border: 2px solid var(--chalk-yellow); border-radius: 50%;
...
tab_qa, tab_worksheet, tab_coverage = st.tabs(
    ["💬 Ask a Question", "📝 Worksheet / Quiz Generator", "✅ Coverage Tracker"]
)
