...
# --------------------------------------------------------------------------
QUESTION_TYPES = {
    "A specific grammar concept": "grammar",
    "A specific book or story (Literature)": "literature",
    "A grade's general overview": "overview",
    "The curriculum's executive summary": "summary",
}


def build_guided_question(chunks_df):
    """Renders cascading selectboxes and returns (question_text, grade_value)."""
    type_label = st.selectbox("1) What do you want to know about?", list(QUESTION_TYPES.keys()))
    ...
    if qtype == "grammar":
        ...
        topic = st.selectbox("3) Grammar concept", topics)
    ...
    if qtype == "literature":
        ...
        grade_choice = st.selectbox("2) Grade (optional)", ["Any grade"] + [int(g) for g in grades])
        grade = None if grade_choice == "Any grade" else grade_choice
        ...
        book = st.selectbox("3) Book / story", books)
        if grade is not None:
