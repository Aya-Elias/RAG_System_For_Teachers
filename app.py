...
# --------------------------------------------------------------------------
# Chat helpers — reformulate a follow-up question into a standalone one
# using recent turns, so retrieval still works for things like "and what
# activities go with it?" after an earlier answer about a specific topic.
# --------------------------------------------------------------------------
def build_standalone_query(chat_history, latest_question, hf_token, max_turns=3):
    if not hf_token or len(chat_history) == 0:
    ...
    rewrite_prompt = (
        "Rewrite the LATEST QUESTION as a fully standalone question that makes "
        "sense with no prior context, using the CONVERSATION below only to resolve "
        "pronouns or implicit references (e.g. 'it', 'that grade', 'those topics'). "
        "If the latest question is already standalone, return it unchanged. "
        "Reply with ONLY the rewritten question, nothing else.\n\n"
        f"CONVERSATION:\n{convo_text}\n\nLATEST QUESTION:\n{latest_question}"
    )
    try:
        client = InferenceClient(model=LLM_CANDIDATES[0], token=hf_token)
        ...
        return rewritten if rewritten else latest_question
    except Exception:
        return latest_question
...
# --------------------------------------------------------------------------
def generate_answer(prompt, hf_token, chat_context=None):
    ...
    chat_context, if given, is a list of {"role", "content"} dicts for the
    last few turns — included so replies can stay conversational (e.g. "as I
    mentioned for grade 3...") without re-deriving facts outside the grounded
...
def build_example_questions(chunks_df, max_examples=3):
    """Builds a few concrete, ready-to-send example questions from the real
    curriculum content (one topic lookup, one book lookup, one activity),
    used as suggestion chips above the chat input."""
    examples = []

    topics = sorted(chunks_df.loc[chunks_df["doc_type"] == "grammar_concept", "grammar_topic"].dropna().unique())
    if topics:
        examples.append(f"Which grade teaches {topics[0]}, and what stage is it introduced at?")

    books = sorted(chunks_df.loc[chunks_df["doc_type"] == "literature", "literature"].dropna().unique())
    if books:
        examples.append(f'Which grade uses "{books[0]}" and at what stage is it assigned?')

    grammar_df = chunks_df[chunks_df["doc_type"] == "grammar_concept"]
    if not grammar_df.empty:
        row = grammar_df.iloc[0]
        examples.append(f"What classroom activity is suggested for teaching {row['grammar_topic']} in grade {int(row['grade'])}?")

    return examples[:max_examples]


# NOTE: no longer called directly by the UI (Tab 1 is now a chat), kept
# because build_example_questions reuses the same category logic above.
...
# --------------------------------------------------------------------------
# Coverage tracker — simple file-based persistence (JSON) so checked
# progress survives across reruns/sessions on the same deployment.
# --------------------------------------------------------------------------
COVERAGE_FILE = "coverage_state.json"
...
# --------------------------------------------------------------------------
# Visual identity — "teacher's chalkboard & exercise book": a dark-green
# chalkboard sidebar (where the lesson settings live) next to a warm
# worksheet-paper main pane with faint ruled lines and a red margin line,
# echoing the two surfaces a primary-school English teacher actually works
# from. Grade badges are styled like a red-pen circle mark on marked work.
# --------------------------------------------------------------------------
def inject_theme():
    st.markdown(
        """
        ...
        h1, h2, h3, .stMarkdown h1, .stMarkdown h2, .stMarkdown h3 {
            font-family: 'Bitter', serif; color: var(--pine); letter-spacing: -0.01em;
        }

        /* Worksheet paper main pane: faint ruled lines + red margin line */
        ...
        .stButton > button, .stDownloadButton > button {
            border-radius: 8px; border: 1px solid var(--pine); font-weight: 600;
        }
        .stButton > button[kind="primary"] {
            background-color: var(--chalk-yellow); color: #2B2B26 !important; border: none;
        }
        [data-testid="stSidebar"] .stButton > button {
            background-color: transparent; border: 1px solid var(--chalk-yellow); color: var(--chalk-yellow) !important;
        }

        /* Chat bubbles */
        [data-testid="stChatMessage"] {
            background-color: #FFFFFF; border-radius: 12px; padding: 4px 8px;
            box-shadow: 0 1px 3px rgba(47,75,60,0.12); margin-bottom: 6px;
        }

        /* Grade badge — red-pen "marked" circle, the page's signature element */
        .grade-badge {
            display: inline-block; font-family: 'Caveat', cursive; font-size: 1.05rem;
            color: var(--margin-red); border: 2px solid var(--margin-red); border-radius: 50%;
...
def grade_badge_html(grade):
    if grade is None:
        return ""
    try:
        return f'<span class="grade-badge">{int(grade)}</span>'
...
# Streamlit UI
# --------------------------------------------------------------------------
st.set_page_config(page_title="Curriculum Chat — Primary English", page_icon="📗", layout="centered")
inject_theme()
st.title("📗 Curriculum Chat")
st.caption("Chat with the Primary English Curriculum Map (Grades 1–6). Answers stay grounded in the curriculum document.")
...
tab_qa, tab_worksheet, tab_coverage = st.tabs(
    ["💬 Chat", "📝 Worksheet / Quiz Generator", "✅ Coverage Tracker"]
)

# ==========================================================================
# TAB 1 — Chat (retrieval-grounded, with follow-up-aware query rewriting)
# ==========================================================================
with tab_qa:
    if "chat_history" not in st.session_state:
        st.session_state.chat_history = []  # [{"role", "content", "sources": df|None}]

    if not st.session_state.chat_history:
        st.caption("Try one of these, or type your own question below:")
        example_questions = build_example_questions(chunks_df)
        cols = st.columns(len(example_questions)) if example_questions else []
        for i, (col, ex_question) in enumerate(zip(cols, example_questions)):
            with col:
                if st.button(ex_question, key=f"ex_{i}", use_container_width=True):
                    st.session_state["pending_question"] = ex_question

    for turn in st.session_state.chat_history:
        avatar = "🧑‍🏫" if turn["role"] == "user" else "📗"
        with st.chat_message(turn["role"], avatar=avatar):
            st.write(turn["content"])
            if turn.get("sources") is not None and not turn["sources"].empty:
                with st.expander("📚 View retrieved sources"):
                    for idx, row in turn["sources"].iterrows():
                        ...
                        if idx != turn["sources"].index[-1]:
                            st.divider()

    typed_question = st.chat_input("Ask about the curriculum (e.g. grammar topics, stories, activities, grades)…")
    ...
    if question and question.strip():
        ...
        with st.chat_message("assistant", avatar="📗"):
            with st.spinner("🔍 Searching the curriculum database…"):
                standalone_question = build_standalone_query(
                    st.session_state.chat_history[:-1], question, hf_token
                )
                retrieved = retrieve_top_k(
                    chunks_df, vectorstore, embedding_model, standalone_question,
                    grade=grade_value, k=k, fetch_k=max(20, k * 4),
                )
                context_text, selected_df = build_context(retrieved)
                prompt = build_grounded_prompt(standalone_question, context_text)
                recent_context = [
                    {"role": turn["role"], "content": turn["content"]}
                    for turn in st.session_state.chat_history[-7:-1]
                ]
                answer, model_used = generate_answer(prompt, hf_token, chat_context=recent_context)
        ...
        st.session_state.chat_history.append(
            {"role": "assistant", "content": answer, "sources": selected_df}
        )
        if pending_question:
            st.rerun()
...
# ==========================================================================
with tab_coverage:
    st.caption("Track which grammar topics and stories you've already taught. Progress is saved automatically.")

    if "coverage_state" not in st.session_state:
        st.session_state.coverage_state = load_coverage_state()
    ...
    total_done = sum(1 for cid in trackable_df["chunk_id"] if st.session_state.coverage_state.get(cid))
    if total_items:
    ...
    for g in coverage_grades:
        ...
        grade_done = sum(1 for cid in grade_items["chunk_id"] if st.session_state.coverage_state.get(cid))
        with st.expander(f"Grade {int(g)}  ·  {grade_done}/{len(grade_items)} covered"):
            for _, item in grade_items.iterrows():
                ...
                current_value = st.session_state.coverage_state.get(cid, False)
                new_value = st.checkbox(label, value=current_value, key=f"cov_{cid}")
                if new_value != current_value:
                    st.session_state.coverage_state[cid] = new_value
                    state_changed = True

    if state_changed:
        save_coverage_state(st.session_state.coverage_state)

    st.divider()
    if st.button("🗑️ Reset coverage progress"):
        st.session_state.coverage_state = {}
        save_coverage_state({})
        st.rerun()

    st.divider()
    st.subheader("Export Coverage Report")
    report_lines = ["# Curriculum Coverage Report", "", f"Overall progress: {total_done}/{total_items} items covered", ""]
    for g in coverage_grades:
        grade_items = trackable_df[trackable_df["grade"] == g]
        grade_done = sum(1 for cid in grade_items["chunk_id"] if st.session_state.coverage_state.get(cid))
        report_lines.append(f"## Grade {int(g)}  ({grade_done}/{len(grade_items)} covered)")
        for _, item in grade_items.iterrows():
            mark = "[x]" if st.session_state.coverage_state.get(item["chunk_id"]) else "[ ]"
